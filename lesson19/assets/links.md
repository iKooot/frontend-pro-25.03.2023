[Promise](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Promise)

[Async function](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Statements/async_function)

[Try catch](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/try...catch?retiredLocale=uk)

[Error](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Error)

[Error types](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Errors)

[HTTP](https://developer.mozilla.org/ru/docs/Web/HTTP)

[Http methods](https://developer.mozilla.org/ru/docs/Web/HTTP/Methods)

[Form data](https://developer.mozilla.org/en-US/docs/Web/API/FormData)

[User-Agent](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/User-Agent)

[HTTP overview](https://developer.mozilla.org/en-US/docs/Web/HTTP/Overview)

[TCP/IP dou](https://dou.ua/forums/topic/33744/)

[Ajax](https://developer.mozilla.org/en-US/docs/Glossary/AJAX?retiredLocale=uk)

[Fetch](https://fetch.spec.whatwg.org/)

[Fetch2](https://developer.mozilla.org/ru/docs/Web/API/Fetch_API/Using_Fetch)

[Fetch3](https://developer.mozilla.org/ru/docs/Web/API/fetch)

[XMLHttpRequest](https://developer.mozilla.org/ru/docs/Web/API/XMLHttpRequest)

[CORS](https://developer.mozilla.org/ru/docs/Web/HTTP/CORS)

[WebSocket](https://developer.mozilla.org/en-US/docs/Web/API/WebSocket?retiredLocale=uk)

[WebSocket2](https://datatracker.ietf.org/doc/html/rfc6455)

[Abort controller](https://developer.mozilla.org/ru/docs/Web/API/AbortController)